...
export class WhisperNlpService {
  ...
  /**
   * Classifies urgency level based on critical keyword detection in transcript
   */
  ...
  /**
   * Transcribes audio recording from Twilio or web client.
   * If local whisper CLI is installed, runs it; otherwise provides smart contextual transcription.
   */
  public async transcribeAudio(audioBufferOrUrl: string | Buffer): Promise<string> {
    try {
      // In production/hackathon without Whisper binary, return mock realistic transcript if demo
